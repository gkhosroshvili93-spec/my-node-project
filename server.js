...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/socialportal';
...
// Encryption Helpers
const ENCRYPTION_KEY = crypto.scryptSync(process.env.ENCRYPTION_KEY || 'my-super-secret-password-123', 'salt', 32);
const IV_LENGTH = 16;
...
app.use(async (req, res, next) => {
    if (req.session.isAuthenticated) {
        try {
            const myNotifications = await Notification.find({ userId: req.session.userEmail }).sort({ timestamp: -1 }).limit(20);
...
app.post("/register", async (req, res) => {
  try {
      const { firstName, lastName, username, email, password, confirmPassword, securityQuestion, securityAnswer } = req.body;

      if (!firstName || !lastName || !username || !email || !password || !confirmPassword || !securityQuestion || !securityAnswer) {
        return res.render("register", { error: "გთხოვთ შეავსოთ ყველა ველი", formData: req.body });
      }

      if (password !== confirmPassword) {
        return res.render("register", { error: "პაროლები არ ემთხვევა", formData: req.body });
      }

      const existingUser = await User.findOne({ $or: [{ email }, { username }] });
      if (existingUser) {
        return res.render("register", { error: "ელ.ფოსტა ან მომხმარებლის სახელი დაკავებულია", formData: req.body });
      }
      ...
      console.error(e);
      require('fs').appendFileSync('error_log.txt', JSON.stringify(e, Object.getOwnPropertyNames(e)) + '\n');

      res.render("register", { error: "სისტემური შეცდომა", formData: req.body || {} });
  }
});
...
app.post("/login", loginLimiter, async (req, res) => {
  try {
      ...
      if (!process.env.SKIP_CAPTCHA) { // Optional generic flag
      ...
      if (!user) {
         return res.render("login", { error: "ელ.ფოსტა ან პაროლი არასწორია", email });
      }
...
app.get("/dashboard", async (req, res) => {
    ...
    try {
        ...
        const userFriends = (currentUser.friends || []).map(email => usersMap[email]).filter(u => u); // full user objects
        ...
        res.render("dashboard", {
            ...
            userFriends,
            friendsData // Needed for sidebar filtering if logic is in EJS
        });
...
app.post("/post/like/:id", async (req, res) => {
    if (!req.session.isAuthenticated) return res.redirect("/login");
    try {
        const post = await Post.findOne({ _id: req.params.id }) || await Post.findOne({ id: req.params.id }); // Try both ID types
        if (post) {
            ...
            if (index === -1) {
                ...
                if (post.userId !== myEmail) {
                    ...
                    const notif = new Notification({
                        userId: post.userId,
                        type: 'like',
                        from: { name: me.firstName + " " + me.lastName, avatar: me.avatar, email: me.email },
...
app.post("/post/comment/:id", async (req, res) => {
    ...
    try {
        ...
        const post = await Post.findOne({ _id: req.params.id }) || await Post.findOne({ id: req.params.id });
        if (post && comment) {
...
app.post("/story/create", upload.single("image"), async (req, res) => {
    ...
    try {
        ...
        const newStory = new Story({
            ...
            authorName: currentUser.firstName + " " + currentUser.lastName,
            authorAvatar: currentUser.avatar,
            image: type === 'photo' && req.file ? "/uploads/" + req.file.filename : "",
            text: type === 'text' ? text : "",
...
app.post("/story/delete/:id", async (req, res) => {
    if (!req.session.isAuthenticated) return res.redirect("/login");
    try {
         await Story.deleteOne({ _id: req.params.id, userId: req.session.userEmail });
    } catch(e) { console.error(e); }
    res.redirect("/dashboard");
});
...
io.on("connection", (socket) => {
    ...
    socket.on("call-user", (data) => {
        io.to(data.userToCall).emit("call-made", { offer: data.offer, socket: socket.id, fromUser: data.fromUser });
    });
...
app.get("/user/:email", async (req, res) => {
    ...
    try {
        ...
        const allPosts = await Post.find({ userId: targetEmail }).sort({ timestamp: -1 });
        // Privacy filter
        ...
        res.render("user-profile", {
            ...
            userPosts,
            isFriend,
            isRequestSent: (currentUser.friendRequestsSent || []).includes(targetEmail),
            isRequestReceived: (currentUser.friendRequestsReceived || []).includes(targetEmail),
...
app.get("/friends/add/:email", async (req, res) => {
    ...
    await User.updateOne({ email: myEmail }, { $addToSet: { friendRequestsSent: targetEmail } });
    await User.updateOne({ email: targetEmail }, { $addToSet: { friendRequestsReceived: myEmail } });
...
app.get("/friends/accept/:email", async (req, res) => {
    ...
    await User.updateOne({ email: myEmail }, { 
...
app.get("/friends", async (req, res) => {
    ...
    const friendsData = {
        [currentUser.email]: {
            ...
            requests_sent: currentUser.friendRequestsSent || [],
            requests_received: currentUser.friendRequestsReceived || [],
            suggestions: users.map(u => u.email).filter(e => e !== currentUser.email && !currentUser.friends.includes(e)) // naive suggestions
        }
    };
